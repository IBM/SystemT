...
/**
 * Consolidation that allows only the first span in the document to pass.
 * 
 */
public class RetainFirstConsImpl extends SortingConsImpl {
  ...
  protected void reallyConsolidate(TupleList sortedInput, TupleList results, MemoizationTable mt) {
    ...
    if (0 == sortedInput.size()) {
      ...
      // order.
      Tuple firstTup = sortedInput.getElemAtIndex(0);
      results.add(firstTup);
    }
  }
}
