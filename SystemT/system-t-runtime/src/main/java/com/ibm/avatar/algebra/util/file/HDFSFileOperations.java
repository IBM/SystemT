...
public final class HDFSFileOperations extends FileOperations {
  ...
  /**
   * This class provides basic file operations for Hadoop distributed file system. While running
   * this class expects hadoop-core*.jar and its dependent jars in the classpath; in addition to
   * these jars, this class also expects the Hadoop configurations: core-site.xml and hdfs-site.xml
   * in the classpath.
   */
  /** Caching the loaded classes */
  private static Class hdfsConfigClazz, hdfsPathClazz, fileSystemClazz;

  /** Hadoop configuration instance, created based on the configuation files in the classpath */
  ...
  /**
   * Flag to signal that the class in initialized; that is, all the Hadoop classes are loaded and
   * handle to HDFS file system is initialized
   */
   ...
   * 
   * @throws Exception if any of the required Hadoop classes missing in the classpath.
   */
  private HDFSFileOperations() throws Exception {
    ...
    if (!initialized) {
      ...
      // create HDFS filesystem instance from the configuration in classpath -
      // FileSystem.get(configuration)
      Method getStaticMethod = fileSystemClazz.getMethod("get", hdfsConfigClazz);
      fileSystem = getStaticMethod.invoke(null, hdfsConfig); // first arg null, because 'get' is a
                                                             // static method
  ...
  /**
   * Method to create HDFS org.apache.hadoop.fs.Path instance for the given URI.
   * 
  ...
  private Object createPath(String uri) throws Exception {
    // Since all abstract methods in HDFSOperations call createPath(), we encode the URI String to a
    // valid URI format
    // here.
  ...
  /**
   * Method to yield the FieldSystem instance that this input {@code filePath} instance belongs to
   * 
   * @param filePath a Path instance wrapping around the file's location
   * @return a FileSystem instance that represents the file system to which the input file path
   *         belongs
   * @throws Exception
   */
