...
public class CreateDictPlanNode extends PlanNode {

  /** Parse tree node for the create dict statement; handles most of the work. */
  CreateDictNode dn;

  public CreateDictPlanNode(CreateDictNode dn) {
    super(new PlanNode[] {});
    if (null == dn) {
      throw new RuntimeException("dn is null");
    }
    this.dn = dn;
  }
  ...
  public PlanNode deepCopyImpl() {
    return new CreateDictPlanNode(dn);
  }

  @Override
  public void dump(PrintWriter stream, int indent) {
    printIndent(stream, indent);
    stream.printf("CreateDictPlanNode\n");
    printIndent(stream, indent + 1);
    stream.printf("Dictionary Name: %s", dn.getDictname());
  }
