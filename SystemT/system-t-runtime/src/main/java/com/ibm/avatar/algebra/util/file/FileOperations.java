...
public abstract class FileOperations {
  ...
  /**
   * Resolves a module path of semicolon-separated "URI strings" (which may be schemeless) into a
   * module path of semicolon-separated URIs with schemes
   * 
   * @param modulePath the module path which may contain URI schemes to resolve
   */
  public synchronized static String resolveModulePath(String modulePathURI) throws Exception {
    // null module path okay -- load from TA application and System classloader
    if (modulePathURI == null) {
      return null;
    }

    if (0 == modulePathURI.length()) {
      throw new TextAnalyticsException("Empty module path");
    }

    try {
      String[] origPaths = modulePathURI.split(Character.toString(Constants.MODULEPATH_SEP_CHAR));
      String[] normalizedPaths = new String[origPaths.length];

      // Resolve scheme-less URIs by discovering their scheme, then normalize them and stitch them
      // back together.
      for (int i = 0; i < origPaths.length; i++) {
        normalizedPaths[i] = normalizeURI(resolvePath(origPaths[i]));
      }

      return StringUtils.join(normalizedPaths, Constants.MODULEPATH_SEP_CHAR);

    } catch (Throwable t) {
      // rewrap into a more specific exception
      throw new URIToFileException(t, modulePathURI);
    }
  }
  ...
  public synchronized static FileSystemType getFileSystemType(String uriString)
    ...
    return getFileSystemType(uri);
  }

  /**
   * Returns the corresponding filesystem type. If the URI has no scheme, auto-discover the correct
   * filesystem type. Current filesystem types supported : local, HDFS, GPFS
   * 
   * @param uri a URI
   * @return a String containing the type of filesystem this URI string refers to
   * @throws TextAnalyticsException
   */
  public synchronized static FileSystemType getFileSystemType(URI uri)
      throws TextAnalyticsException {
